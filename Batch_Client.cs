namespace Microsoft.Azure.Batch.Samples.Client
{
    ...
    public class Program
    {
        ...
        static int nodeNumber = int.Parse(ConfigurationManager.AppSettings["PoolSize"]);                       // Number of VMs to run in parallel
        static int tasksPerNode = 1;                                                                           // Number of tasks run subsequently on each node, DO NOT CHANGE
        static int taskNumber = tasksPerNode * nodeNumber;                                                     // Number of Tasks to be added to the job
        ...
        private static async Task MainAsync()
        {
            ...
            string applicationDirectory = ConfigurationManager.AppSettings["applicationDirectory"];
            ...
            string sourceData = ConfigurationManager.AppSettings["dataFilePath"];

            // Paths to the executable and its dependencies that will be executed by the tasks
            List<string> applicationFilePaths = new List<string>();
            applicationFilePaths.AddRange(Directory.GetFiles(@applicationDirectory));

            Console.WriteLine("Output folder set to {0}", outputDirectory);
            Console.WriteLine();

            taskNumber = int.Parse(ConfigurationManager.AppSettings["TaskNumber"]);

            List<string> inputFilePaths = new List<string>();
            inputFilePaths.AddRange(Directory.GetFiles(@inputDirectory));
            ...
            Console.WriteLine("Tasknumber = {0}", taskNumber);
            ...
            using (BatchClient batchClient = BatchClient.Open(cred))
            {
                ...
                foreach (string file in inputFilePaths)
                    Console.WriteLine("Inputfile: file {0}", file);

                Console.WriteLine("TaskNumber is currently {0}", taskNumber);
                ...
                await AddTasksAsync(batchClient, JobId, inputFiles, taskNumber);

                //await AddTasksAsync(batchClient, JobId, inputFiles, outputContainerSasUrl);

                int TimeOutLimit = int.Parse(ConfigurationManager.AppSettings["TimeOutLimit"]);

                // Monitor task success/failure, specifying a maximum amount of time to wait for the tasks to complete
                await MonitorTasks(batchClient, JobId, TimeSpan.FromMinutes(TimeOutLimit));

                // Download the task output files from the output Storage container to a local directory
                await DownloadBlobsFromContainerAsync(blobClient, outputContainerName, outputDirectory);
        ...
        private static async Task<List<ResourceFile>> UploadFilesToContainerAsync(CloudBlobClient blobClient, string inputContainerName, List<string> filePaths)
        {
            ...
            // Upload Batch resources (if the user so chooses)
            Console.WriteLine();
            foreach (string filePath in filePaths)
            {
                Console.WriteLine(filePath);
            }
            Console.Write("Upload files to container [{0}]? [yes] no: ", inputContainerName);
            string response = Console.ReadLine().ToLower();
          //  if (response != "n" && response != "no")
            foreach (string filePath in filePaths)
            {
                resourceFiles.Add(await UploadFileToContainerAsync(blobClient, inputContainerName, filePath, response));
            }
        ...
        private static async Task<ResourceFile> UploadFileToContainerAsync(CloudBlobClient blobClient, string containerName,
        ...
        private static async Task CreatePoolAsync(BatchClient batchClient, string poolId, IList<ResourceFile> resourceFiles)
        {
            ...
            //  int PoolSize = int.Parse(ConfigurationManager.AppSettings["PoolSize"]);
            int PoolSize = nodeNumber;
            string NodeSize = ConfigurationManager.AppSettings["NodeSize"];
            // Create the unbound pool. Until we call CloudPool.Commit() or CommitAsync(), no pool is actually created in the
            // Batch service. This CloudPool instance is therefore considered "unbound," and we can modify its properties.
            CloudPool pool = batchClient.PoolOperations.CreatePool(
                poolId: poolId,
                targetDedicated: PoolSize,        // Number of compute nodes
                virtualMachineSize: NodeSize,     // Node instance size
                osFamily: "4");                   // "4" = Windows Server 2012 R2
...
                CommandLine = "cmd /c starttask.cmd",
        ...
        private static async Task<List<CloudTask>> AddTasksAsync(BatchClient batchClient, string jobId, List<ResourceFile> inputFiles, int taskNumber)
        {
            ...
            for (int i = 1; i <= taskNumber; i++)
            {
                ...
                //string taskCommandLine = String.Format("cmd /c NLRTask.cmd");


                string taskId = "taskNumber" + i.ToString();
                string taskCommandLine = "cmd /c task.cmd";

                CloudTask task = new CloudTask(taskId, taskCommandLine);
                task.RunElevated = true;
                task.ResourceFiles = inputFiles.GetRange(i - 1, 1);
                tasks.Add(task);
                
                Console.WriteLine(i.ToString() + " -> " + taskCommandLine);
            };
        ...
        private static async Task DeleteContainerAsync(CloudBlobClient blobClient, string containerName)
        {
            ...
            Console.WriteLine();
            Console.Write("Delete container [{0}]? [yes] no: ", containerName);
            string response = Console.ReadLine().ToLower();
            if (response != "n" && response != "no")
            {
